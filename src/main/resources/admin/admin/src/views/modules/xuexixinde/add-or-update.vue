    ...
    >
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="学习内容" prop="xuexineirong">
                <editor 
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.xuexineirong" label="学习内容" prop="xuexineirong">
                    <span v-html="ruleForm.xuexineirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.xuexineirong = this.ruleForm.xuexineirong.replace(reg,'../../../springboot0w65d/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xuexixinde/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xuexixindeCrossAddOrUpdateFlag = false;
						 this.parent.search();

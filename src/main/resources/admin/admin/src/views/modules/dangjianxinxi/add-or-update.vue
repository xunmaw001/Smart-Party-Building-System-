<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="责编" prop="zebian">
          <el-input v-model="ruleForm.zebian" 
              placeholder="责编" clearable  :readonly="ro.zebian"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="责编" prop="zebian">
              <el-input v-model="ruleForm.zebian" 
                placeholder="责编" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.dangzhibumingcheng!=''&&json.dangzhibumingcheng){
                this.ruleForm.dangzhibumingcheng = json.dangzhibumingcheng
	    		this.ro.dangzhibumingcheng = true;
		}
        } else {
	...
	this.ruleForm.zhengwen = this.ruleForm.zhengwen.replace(reg,'../../../springboot0w65d/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `dangjianxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.dangjianxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();

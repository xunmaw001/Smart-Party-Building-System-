<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="党支部名称" prop="dangzhibumingcheng">
          <el-select :disabled="ro.dangzhibumingcheng" @change="dangzhibumingchengChange" v-model="ruleForm.dangzhibumingcheng" placeholder="请选择党支部名称">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.dangzhibumingcheng" label="党支部名称" prop="dangzhibumingcheng">
              <el-input v-model="ruleForm.dangzhibumingcheng" 
                placeholder="党支部名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.shenqingfujian" label="申请附件" prop="shenqingfujian">
          <file-upload
          tip="点击上传申请附件"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.shenqingfujian?ruleForm.shenqingfujian:''"
          @change="shenqingfujianUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.shenqingfujian" label="申请附件" prop="shenqingfujian">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.shenqingfujian)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="申请说明" prop="shenqingshuoming">
                <editor 
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.shenqingshuoming" label="申请说明" prop="shenqingshuoming">
                    <span v-html="ruleForm.shenqingshuoming"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='dangzhibumingcheng'){
            this.ruleForm.dangzhibumingcheng = obj[o];
	    this.ro.dangzhibumingcheng = true;
            continue;
          }
          if(o=='yonghuzhanghao'){
	    ...
	    this.ro.shenqingshijian = true;
          ...
          if(o=='shenqingfujian'){
            this.ruleForm.shenqingfujian = obj[o];
	    this.ro.shenqingfujian = true;
            continue;
          }
    ...
    // 下二随
    dangzhibumingchengChange () {
      this.$http({
        url: `follow/dangzhibu/dangzhibumingcheng?columnValue=`+ this.ruleForm.dangzhibumingcheng,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
        } else {
	...
	this.ruleForm.shenqingshuoming = this.ruleForm.shenqingshuoming.replace(reg,'../../../springboot0w65d/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.shenqingfujian!=null) {
		this.ruleForm.shenqingfujian = this.ruleForm.shenqingfujian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `rudangshenqing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.rudangshenqingCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    shenqingfujianUploadChange(fileUrls) {
	this.ruleForm.shenqingfujian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{

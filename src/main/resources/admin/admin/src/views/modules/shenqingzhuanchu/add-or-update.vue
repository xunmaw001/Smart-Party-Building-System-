...
export default {
  ...
  methods: {
	...
	this.ruleForm.shenqingbeizhu = this.ruleForm.shenqingbeizhu.replace(reg,'../../../springboot0w65d/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shenqingzhuanchu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shenqingzhuanchuCrossAddOrUpdateFlag = false;
						 this.parent.search();
